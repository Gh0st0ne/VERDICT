...
public class Vdm2Csv {
     ...
     * @param model name i.e., project directory name 1. Initialize tables with headers if they have
     *     static headers 2. Build tables for STEM: ScnCompProps.csv, ScnConnections.csv,
     *     ScnBusBindings.csv 3. Build tables for Soteria++: CompDep.csv, Mission.csv, CompSaf.csv,
     *     Events.csv, ScnCompProps.csv, ScnConnections.csv 4. Output the csv files
     */
    public void execute(Model vdm, String stemDir, String soteriaDir, String modelName) {
        ...
        Map<String, HashSet<String>> propToConnections =
                new HashMap<>(); // map(property_name, hash_set_of_connections)
        // create map for connection names to attributes names and attribute names to attribute
        // value
        Map<String, HashMap<String, String>> connectionAttributesMap = new HashMap<>();
        Map<String, HashSet<String>> propToCompInsts =
                new HashMap<>(); // map(property_name, hash_set_of_components)
        ...
        // pre-process component implementations info in the VDM and update maps propToConnections,
        // connectionAttributesMap, compToCompImpl, compInstAttributesMap
        // these maps will be used to build tables scnConnTable and scnCompPropsTable
        preprocessCompImpls(
                compImpls,
                modelName,
                propToConnections,
                connectionAttributesMap,
                compToCompImpl,
                propToCompInsts,
                compInstAttributesMap,
                connectionDestToSourceMap);
        Table scnConnTable =
     ...
     */
    private void updateTablesWithDataFromVDM(
            Model inputVDM,
            String modelName,
            Table eventsTable,
            Table compDepTable,
        ...
        updateCyberSafetyReqsMaps(
                inputVDM.getCyberReq(), cyberReqsMap, inputVDM.getSafetyReq(), safetyReqsMap);
        // process mission in VDM and update missions table using safety/cyber req definitions in
        // maps
        processMissionReqs(
                inputVDM.getMission(),
                cyberReqsMap,
    ...
    private void preprocessCompImpls(
            List<ComponentImpl> compImpls,
            String scenario,
            Map<String, HashSet<String>> propToConnections,
            Map<String, HashMap<String, String>> connectionAttributesMap,
            Map<String, String> compToCompImpl,
            Map<String, HashSet<String>> propToCompInsts,
            Map<String, HashMap<String, String>> compInstAttributesMap,
            Map<String, List<ConnectionEnd>> connectionDestToSourceMap) {
        for (ComponentImpl compImpl : compImpls) {
            if (compImpl.getBlockImpl() != null) {
                // process connections
                List<verdict.vdm.vdm_model.Connection> compConnections =
                        compImpl.getBlockImpl().getConnection();
                for (verdict.vdm.vdm_model.Connection compConnection1 : compConnections) {
                    // -- Get all property names associated with connections --> need it for headers
                    // in connections csv
                    ...
                    // -- Create a map of connections that have a data-port as destination rather
                    // than subcomponent.data-port
                ...
                List<verdict.vdm.vdm_model.ComponentInstance> compInstances =
                        compImpl.getBlockImpl().getSubcomponent();
                for (verdict.vdm.vdm_model.ComponentInstance compInst : compInstances) {
                    ...
                    for (GenericAttribute attr : compInstAttributesList) {
                        // --update attribute to attribute value mapping for each component
                        // attribute
                        compInstAttributes.put(attr.getName(), attr.getValue().toString());
                        // -- Get all property names associated with components --> need it for
                        // headers in components csv
    ...
    private Table updateConnectionsTable(
            List<ComponentImpl> compImpls,
            String scenario,
            Map<String, HashSet<String>> propToConnections,
            Map<String, HashMap<String, String>> connectionAttributesMap,
            ...
            if (compImpl.getBlockImpl() != null) {
                ...
                for (verdict.vdm.vdm_model.Connection compConnection2 : compConnections) {
                    updateConnectionTable(
                            compConnection2.getName(),
                            compConnection2.getQualifiedName(),
                            compConnection2.getSource(),
                            compConnection2.getDestination(),
                            scnConnTable,
                            connectionAttributesMap,
                            scenario,
                            compImpl,
    ...
    private void updateConnectionTable(
            String connName,
            String connQualName,
            verdict.vdm.vdm_model.ConnectionEnd source,
            verdict.vdm.vdm_model.ConnectionEnd destination,
            Table scnConnTable,
            Map<String, HashMap<String, String>> connectionAttributesMap,
            String scenario,
        ...
        scnConnTable.addValue(
                replaceColonsWithUnderscore(connQualName)); // connection sanitized qualified name
        ...
        if (source.getComponentPort() != null) {
            ...
            // remove port name from fullQualNameSrcCompPort and text before "::" and after "." to
            // get the source component name
            String compName =
                    fullQualNameSrcCompPort.substring(
                            fullQualNameSrcCompPort.indexOf(':') + 2,
                            fullQualNameSrcCompPort.indexOf('.'));
            if (!compToCompImpl.containsKey(compName)) {
                throw new RuntimeException(
                        "Unable to find Component Implementation corresponding to " + compName);
            }
            ...
            scnConnTable.addValue(
                    replaceColonsWithUnderscore(
                            sourceInstQualName)); // SanitizedSrcCompInstQualifiedName
            scnConnTable.addValue(
                    sourceInstQualName.substring(
                            0, compQualName.indexOf(':'))); // SrcCompInstPackage
            CompInstancePort srcCompPort = source.getSubcomponentPort();
            // get the component name
            scnConnTable.addValue(
                    srcCompPort.getSubcomponent().getSpecification().getName()); // SrcComp
            ...
            throw new RuntimeException(
                    "Connection source has null values foe component port and subcomponent port");
        }
        if (destination.getComponentPort() != null) {
            ...
            // get the component name
            // remove port name from fullQualNameDestCompPort and text before "::" and after "." to
            // get the destination component name
            String compName =
                    fullQualNameDestCompPort.substring(
                            fullQualNameDestCompPort.indexOf(':') + 2,
                            fullQualNameDestCompPort.indexOf('.'));
            if (!compToCompImpl.containsKey(compName)) {
                throw new RuntimeException(
                        "Unable to find Component Implementation corresponding to " + compName);
            }
            ...
            scnConnTable.addValue(
                    replaceColonsWithUnderscore(
                            destInstQualName)); // SanitizedDestCompInstQualifiedName
            scnConnTable.addValue(
                    destInstQualName.substring(
                            0, compQualName.indexOf(':'))); // DestCompInstPackage
            ...
            throw new RuntimeException(
                    "Connection destination has null values foe component port and subcomponent port");
        }
        ...
        for (String propName : propToConnections.keySet()) {
            if (synthesis
                    && DefenseProperties.MBAA_CONN_DEFENSE_PROPERTIES_SET.contains(propName)) {
                // this fools stem - --code synonymous to code by William in the previous
                // aadl2csvtranslator
                scnConnTable.addValue("9");
            } else if (connAttrMap.containsKey(
                    propName)) { // check if the connection has that property - add it to csv if it
                // does
    ...
    private Table updateBusBindingsTable(
            List<ComponentImpl> compImpls, String scenario, Map<String, String> compToCompImpl) {
            ...
            if (compImpl.getBlockImpl() != null) {
                ...
                for (verdict.vdm.vdm_model.Connection compConnection2 : compConnections) {
                    if (compConnection2.getActualConnectionBinding() != null) {
                        String actualConnectionBinding =
                                compConnection2.getActualConnectionBinding();
                        scnBusBindingsTable.addValue(scenario);
                        String compQualName = compImpl.getType().getId();
                        scnBusBindingsTable.addValue(compQualName); // QualifiedName
                        scnBusBindingsTable.addValue(
                                replaceColonsWithUnderscore(
                                        compQualName)); // SanitizedQualifiedName
                        scnBusBindingsTable.addValue(
                                compQualName.substring(
                                        0, compQualName.indexOf(':'))); // PackageName
                        scnBusBindingsTable.addValue(compImpl.getType().getName()); // Comp
                        scnBusBindingsTable.addValue(
                                compImpl.getName()); // Impl - component implementation
                        ...
                        String bindingSubStr =
                                actualConnectionBinding.substring(
                                        0,
                                        actualConnectionBinding.lastIndexOf(
                                                ".")); // everything except the instance name
                        String implName =
                                bindingSubStr.substring(
                                        bindingSubStr.lastIndexOf(":") + 1, bindingSubStr.length());
                        if (!compImplToComp.containsKey(implName)) {
                            throw new RuntimeException(
                                    "Unable to find Component corresponding to Implementation "
                                            + implName);
                        }
                        scnBusBindingsTable.addValue(
                                compImplToComp.get(implName)); // ActualConnectionBindingSrcComp
                        ...
                        scnBusBindingsTable.addValue(
                                replaceColonsWithUnderscore(
                                        actualConnectionBinding)); // SanitizedActualConnectionBindingSrcBusInstName
                        scnBusBindingsTable.addValue(
                                actualConnectionBinding.substring(
                                        actualConnectionBinding.lastIndexOf('.') + 1,
                                        actualConnectionBinding
                                                .length())); // ActualConnectionBindingSrcBusInst
                        ...
                        scnBusBindingsTable.addValue(
                                compImpl.getType()
                                        .getName()); // ActualConnectionBindingDestConnComp
                        scnBusBindingsTable.addValue(
                                compImpl.getName()); // ActualConnectionBindingDestConnImpl
                        scnBusBindingsTable.addValue(""); // ActualConnectionBindingDestConnCompInst
                        scnBusBindingsTable.addValue(
                                replaceColonsWithUnderscore(
                                        compConnection2
                                                .getQualifiedName())); // SanitizedActualConnectionBindingDestConnName
                        scnBusBindingsTable.addValue(
                                compConnection2.getName()); // ActualConnectionBindingDestConn
    ...
    private Table updateCompInstTable(
            List<ComponentImpl> compImpls,
            String scenario,
            Map<String, HashSet<String>> propToCompInsts,
            Map<String, HashMap<String, String>> compInstAttributesMap,
    ...
    private void updateCompPropsTable(
            String compInstName,
    ...
    private void updateCyberSafetyReqsMaps(
            List<verdict.vdm.vdm_model.CyberReq> cyberReqs,
            Map<String, verdict.vdm.vdm_model.CyberReq> cyberReqsMap,
            List<verdict.vdm.vdm_model.SafetyReq> safetyReqs,
            Map<String, verdict.vdm.vdm_model.SafetyReq> safetyReqsMap) {
    ...
    private void processMissionReqs(
            List<verdict.vdm.vdm_model.Mission> missionReqs,
            Map<String, verdict.vdm.vdm_model.CyberReq> cyberReqsMap,
            Map<String, verdict.vdm.vdm_model.SafetyReq> safetyReqsMap,
            Table missionTable,
            String scenario,
        ...
        for (verdict.vdm.vdm_model.Mission missionReq : missionReqs) {
            List<String> cyberReqIds =
                    missionReq.getCyberReqs(); // this may include safety requirements
            for (String cyberReqId : cyberReqIds) {
                ...
                if (cyberReqsMap.containsKey(cyberReqId)) {
                    ...
                    if (cyberReqCondition.getKind()
                            != null) { // if condition is an OR expression get individual
                        // sub-expressions
                        if (cyberReqCondition.getKind().toString().equalsIgnoreCase("Or")) {
                            ...
                            for (CyberExpr subCyberCond : subCyberCondList) {
                                updateCyberMissionsTable(
                                        missionTable,
                                        scenario,
                                        missionReq.getId(),
                                        cyberReqId,
                                        cyberReqDef.getCia(),
                                        cyberReqDef.getSeverity(),
                                        subCyberCond,
                                        connectionDestToSourceMap);
                            }
                        } else if (cyberReqCondition.getKind().toString().equalsIgnoreCase("And")) {
                            updateAndExprCyberMissionsTable(
                                    missionTable,
                                    scenario,
                                    missionReq.getId(),
                                    cyberReqId,
                            ...
                            throw new RuntimeException(
                                    "Expression used as condition in Cyber Requirement is not supported.");
                        }
                    ...
                    // get the condition of the cyberReq
                    verdict.vdm.vdm_model.SafetyReqExpr safetyReqCondition =
                            safetyReqDef.getCondition();
                    // condition could be an OR expression containing multiple expressions
                    // or just single port and IA
                    if (safetyReqCondition.getKind()
                            != null) { // if condition is an OR expression get individual
                        // sub-expressions
                        if (safetyReqCondition.getKind().toString().equalsIgnoreCase("Or")) {
                            List<verdict.vdm.vdm_model.SafetyReqExpr> subSafetyExprCondList =
                                    safetyReqCondition.getOr().getExpr();
                            for (verdict.vdm.vdm_model.SafetyReqExpr subCyberCond :
                                    subSafetyExprCondList) {
                                updateSafetyMissionsTable(
                                        missionTable,
                                        scenario,
                                        missionReq.getId(),
                                        cyberReqId,
                                        safetyReqDef.getTargetProbability(),
                                        subCyberCond,
                                        connectionDestToSourceMap);
                            }
                        } else if (safetyReqCondition
                                .getKind()
                                .toString()
                                .equalsIgnoreCase("And")) {
                            updateAndExprSafetyMissionsTable(
                                    missionTable,
                                    scenario,
                                    missionReq.getId(),
                                    cyberReqId,
                            ...
                            throw new RuntimeException(
                                    "Expression used as condition in Safety Requirement is not supported.");
                        }
                    } else { // if condition has single port and CIa
                        updateSafetyMissionsTable(
                                missionTable,
                    ...
                    throw new RuntimeException(
                            "Undefined cyber or safety requirement used in mission requirement.");
                }
            }
        }
    }

    private void updateAndExprCyberMissionsTable(
            Table missionTable,
            String scenario,
            String missionReqId,
            String cyberReqId,
    ...
    private void updateCyberMissionsTable(
            Table missionTable,
            String scenario,
            String missionReqId,
            String cyberReqId,
            verdict.vdm.vdm_model.CIA cyberCIA,
            verdict.vdm.vdm_model.Severity cyberSeverity,
            CyberExpr cyberReqCondition,
            Map<String, List<ConnectionEnd>> connectionDestToSourceMap) {
        // get port's linked source instance
        if (connectionDestToSourceMap.containsKey(cyberReqCondition.getPort().getName())) {
            List<ConnectionEnd> linkedSourcePorts =
                    connectionDestToSourceMap.get(cyberReqCondition.getPort().getName());
            for (ConnectionEnd linkedSourcePort : linkedSourcePorts) {
                if (linkedSourcePort.getSubcomponentPort() != null) {
                    ...
                    missionTable.addValue(
                            formatToSmall(cyberReqCondition.getPort().getCia().name()));
                    missionTable.addValue("Cyber");
                    missionTable.capRow();
                } else {
            ...
            throw new RuntimeException(
                    "Missing component instance dependency. "
                            + cyberReqCondition.getPort().getName()
                            + " is not linked to a source port");
        }
    }

    private void updateAndExprSafetyMissionsTable(
            Table missionTable,
            String scenario,
            String missionReqId,
            String cyberReqId,
            String targetProbability,
            SafetyReqExpr subCyberCond,
            Map<String, List<ConnectionEnd>> connectionDestToSourceMap) {
        missionTable.addValue(scenario);
        missionTable.addValue(sanitizeValue(missionReqId));
        missionTable.addValue(""); // MissionReq
    ...
    private void updateSafetyMissionsTable(
            Table missionTable,
            String scenario,
            String missionReqId,
            String cyberReqId,
            String probability,
            SafetyReqExpr safetyReqCondition,
            Map<String, List<ConnectionEnd>> connectionDestToSourceMap) {
        if (connectionDestToSourceMap.containsKey(safetyReqCondition.getPort().getName())) {
            List<ConnectionEnd> linkedSourcePorts =
                    connectionDestToSourceMap.get(safetyReqCondition.getPort().getName());
            for (ConnectionEnd linkedSourcePort : linkedSourcePorts) {
                if (linkedSourcePort.getSubcomponentPort() != null) {
                    CompInstancePort destCompPort = linkedSourcePort.getSubcomponentPort();
                    missionTable.addValue(scenario);
                    ...
                    // get CIA and add it to table
                    missionTable.addValue(
                            formatToSmall(safetyReqCondition.getPort().getIa().name()));
                    missionTable.addValue("Safety");
                    missionTable.capRow();
                } else {
                    throw new RuntimeException("Linked Source Port has no instance information");
            ...
            throw new RuntimeException(
                    "Missing component instance dependency. "
                            + safetyReqCondition.getPort().getName()
                            + " is not linked to a source port");
        }
    }

    private void processComponentTypes(
            List<ComponentType> compTypes,
            Table eventsTable,
            Table compDepTable,
            Table compSafTable) {
        // each ComponentType contains id, name, compCateg, List<Port>, ContractSpec,
        // List<CyberRel>, List<SafetyRel>, List<Event>
        for (ComponentType compType : compTypes) {
            ...
            updateEventsTable(
                    eventsTable, compQualName, packageName, compTypeName, compType.getEvent());
            updateCompDepTable(
                    compDepTable, compQualName, packageName, compTypeName, compType.getCyberRel());
            updateCompSafTable(
                    compSafTable, compQualName, packageName, compTypeName, compType.getSafetyRel());
        }
    }

    private void updateCompSafTable(
            Table compSafTable,
            String qualNameComp,
            String packageName,
            String compTypeName,
    ...
    private void updateCompSafTable(
            ...
            String packageName,
            String compTypeName,
            verdict.vdm.vdm_model.SafetyRelExpr safetyRelExpr,
            IAPort outputIAPort) {
        if (safetyRelExpr.getKind() == null) {
            if (safetyRelExpr.getFault() != null) {
                ...
                throw new RuntimeException(
                        "Safety Expression has null values for expr-kind and event-happens/fault");
            }
            ...
            throw new RuntimeException(
                    "Expression used as Safety Relation input is not supported.");
        }
    }
    // Assuming that the sub-expression in the safety relation is AND over port-IA types
    private void updateCompSafTableWithAndExpr(
            Table compSafTable,
            String qualNameComp,
            String packageName,
            String compTypeName,
            SafetyRelExpr safetyRelExpr,
            IAPort outputIAPort) {
    ...
    private void updateCompDepTable(
            Table compDepTable,
            String qualNameComp,
            String packageName,
            String compTypeName,
    ...
    private void updateCompDepTable(
            ...
            String packageName,
            String compTypeName,
            CyberExpr inputCyberExpr,
            CIAPort outputCIAPort) {
        if (inputCyberExpr.getKind() == null) { // expression is not an AND, OR, NOT expression
            ...
            updateCompDepTableWithAndCyberExpr(
                    compDepTable,
                    qualNameComp,
                    packageName,
                    compTypeName,
    ...
    private void updateCompDepTableWithAndCyberExpr(
            Table compDepTable,
            String qualNameComp,
            String packageName,
            String compTypeName,
        ...
        if (inputCyberExpr.getKind() == null) {
            if (portsCIAs[0].equalsIgnoreCase("")) {
                ...
                portsCIAs[1] =
                        portsCIAs[1]
                                + ";"
                                + formatToSmall(inputCyberExpr.getPort().getCia().name());
            }
        } else if (inputCyberExpr.getKind().toString().equalsIgnoreCase("And")) {
    ...
    private void getPortsAndCIAsForMission(
            CyberExpr inputCyberExpr,
            String[] portsCIAs,
            Map<String, List<ConnectionEnd>> connectionDestToSourceMap) {
        if (inputCyberExpr.getKind() == null) {
            ...
            if (connectionDestToSourceMap.containsKey(inputCyberExpr.getPort().getName())) {
                List<ConnectionEnd> linkedSourcePorts =
                        connectionDestToSourceMap.get(inputCyberExpr.getPort().getName());
                if (linkedSourcePorts.size() > 1) {
                    throw new RuntimeException(
                            "Multiple Linked Source Ports is unexpected for ports in cyber expression.");
                }
                ...
                throw new RuntimeException(
                        "Missing component instance dependency. "
                                + inputCyberExpr.getPort().getName()
                                + " is not linked to a source port");
            }
        } else if (inputCyberExpr.getKind().toString().equalsIgnoreCase("And")) {
            List<CyberExpr> subInpCyberList = inputCyberExpr.getAnd().getExpr();
    ...
    private void getPortsAndIAsForMission(
            SafetyReqExpr inputSafetyExpr,
            String[] portsIAs,
            Map<String, List<ConnectionEnd>> connectionDestToSourceMap) {
        if (inputSafetyExpr.getKind() == null) {
            ...
            if (connectionDestToSourceMap.containsKey(inputSafetyExpr.getPort().getName())) {
                List<ConnectionEnd> linkedSourcePorts =
                        connectionDestToSourceMap.get(inputSafetyExpr.getPort().getName());
                if (linkedSourcePorts.size() > 1) {
                    throw new RuntimeException(
                            "Multiple Linked Source Ports is unexpected for ports in cyber expression.");
                }
                if (linkedSourcePorts.get(0).getSubcomponentPort() != null) {
                    ...
                    if (portsIAs[0].equalsIgnoreCase("")) {
                        ...
                        portsIAs[2] =
                                portsIAs[2]
                                        + ";"
                                        + formatToSmall(inputSafetyExpr.getPort().getIa().name());
                    }
                } else {
                    throw new RuntimeException("Linked Source Port has no instance information");
                }
                ...
                throw new RuntimeException(
                        "Missing component instance dependency. "
                                + inputSafetyExpr.getPort().getName()
                                + " is not linked to a source port");
            }
    ...
    private void updateEventsTable(
            ...
            String packageName,
            String compTypeName,
            List<verdict.vdm.vdm_model.Event> events) {
