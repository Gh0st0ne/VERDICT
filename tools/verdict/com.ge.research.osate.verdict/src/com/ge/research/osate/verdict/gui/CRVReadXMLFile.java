...
public class CRVReadXMLFile {
                ...
                if (nNode.getNodeType() == Node.ELEMENT_NODE) {
                    ...
                    newProperty.setSource(
                            eElement.getElementsByTagName("Answer")
                                    ...
                                    .getNamedItem("source")
                                    .getTextContent());
                    newProperty.setAnswer(
                            eElement.getElementsByTagName("Answer").item(0).getTextContent());
                    if (eElement.getElementsByTagName("Answer")
                            .item(0)
                            .getTextContent()
                            .equals("falsifiable")) {
                        newProperty.setCntExample(
                                extractCE(eElement.getElementsByTagName("CounterExample").item(0)));
                        newProperty.setBlameAssignment(
                                extractBlameAssignment(
                                        eElement.getAttribute("name"), doc, fileName2));
                    } else if (eElement.getElementsByTagName("Answer")
                            .item(0)
                            .getTextContent()
                            .equals("unknown")) {
                        try {
                            newProperty.setValidTill(
                                    eElement.getElementsByTagName("TrueFor")
                                            .item(0)
                                            .getTextContent());
                        } catch (Exception e) {
                            // do nothing
    ...
    protected BlameAssignmentInfo extractBlameAssignment(
            String propertyName, Document doc, String fileName2) {
            ...
            if (nMinA != null) {
                ...
                for (int i = 0; i < componentList.getLength(); i++) {
                    ...
                    String componentName =
                            nComponent.getAttributes().getNamedItem("ComponentID").getTextContent();
                    if ("true"
                            .equals(
                                    nComponent
                                            .getAttributes()
                                            .getNamedItem("Compromised")
                                            .getTextContent())) {
                ...
                for (int i = 0; i < threatList.getLength(); i++) {
                    ...
                    if (nThreat != null) {
                        NodeList descriptionsList =
                                ((Element) nThreat).getElementsByTagName("AttackDescription");
