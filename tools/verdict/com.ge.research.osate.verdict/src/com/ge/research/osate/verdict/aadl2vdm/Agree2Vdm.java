...
public class Agree2Vdm {
    ...
    public Model execute(File inputDir) {
        ...
        // using preprocessAadlFiles method in this class, to get objects in the aadl files in the
        // directory along with the imported aadl files
        List<EObject> objectsFromAllFiles = preprocessAadlFiles(inputDir);
        // using preprocessAadlFiles method from AADL2VDM to get objects from the aadl files in the
        // directory alone
     ...
     * @return a populated VDM model This method is invoked after preprocessAadlFiles and
     *     populateVDMFromAadlObjects, i.e., the aadl files are first parsed, translated into
     *     objects in preprocessAadlFiles(), then VDM is populated with objects corresponding to
     *     AADL constructs in populateVDMFromAadlObjects(), then this method is invoked to populate
     *     the agree constructs in the vdm model
     */
    ...
    private Model translateAgreeAnnex(
            List<SystemType> systemTypes,
            Model model,
            HashSet<String> dataTypeDecl,
            HashSet<String> nodeDecl) {
        ...
        for (SystemType sysType : systemTypes) {
            ...
            for (AnnexSubclause annex : sysType.getOwnedAnnexSubclauses()) {
                if (annex.getName().equalsIgnoreCase("agree")) {
                    ...
                    AgreeContractSubclause agreeAnnex =
                            (AgreeContractSubclause) ddASC.getParsedAnnexSubclause();
                    // populating agree contracts in the vdm component type -- SHOULD ADD THIS CODE
                    // TO AADL2VDM
                    verdict.vdm.vdm_lustre.ContractSpec contractSpec =
                            new verdict.vdm.vdm_lustre.ContractSpec();
                    ...
                    for (EObject clause : annexContents) {
                        ...
                        for (SpecStatement specStatement : specStatements) {
                            if (specStatement instanceof EqStatement) {
                                ...
                                SymbolDefinition symbDef =
                                        translateEqStatement(eqStmt, model, dataTypeDecl, nodeDecl);
                                ...
                                GuaranteeStatement guaranteeStmt =
                                        (GuaranteeStatement) specStatement;
                                ContractItem contractItem =
                                        translateGuaranteeStatement(
                                                guaranteeStmt, dataTypeDecl, nodeDecl, model);
                                ...
                                ContractItem contractItem =
                                        translateAssumeStatement(
                                                assumeStmt, dataTypeDecl, nodeDecl, model);
                                contractSpec.getAssume().add(contractItem);
                            } else {
                                System.out.println(
                                        "Element not recognizable" + clause.eContents().toString());
                            }
                        }
                    }
                    List<ComponentType> vdmComponentTypes = model.getComponentType();
                    for (ComponentType vdmComponentType : vdmComponentTypes) {
                        // populating agree contract details in the corresponding componentType
                        // instance in vdm
    ...
    private ContractItem translateAssumeStatement(
            AssumeStatement assumeStmt,
            HashSet<String> dataTypeDecl,
            HashSet<String> nodeDecl,
            Model model) {
        ...
        Expression vdmlustrExpr =
                getVdmExpressionFromAgreeExpression(agreeExpr, dataTypeDecl, nodeDecl, model);
    ...
    private SymbolDefinition translateEqStatement(
            EqStatement eqStmt,
            Model model,
            HashSet<String> dataTypeDecl,
            HashSet<String> nodeDecl) {
        ...
        for (Arg lhsArg : lhsArgs) { // left side has the variable names along with their types
            ...
            // need to parse the type of the variable and should map to appropriate DataType value
            // (plainType, subrangeType, arrayType, tupleType, enumType, recordType,
            // userDefinedType) of the symbol
            Type type = lhsArg.getType();
            // set just name of the type - but make sure you add the type declaration to the vdm
            // model if not already added
    ...
    private void translateAgreeDataTypeToVdmDataType(
            Type type, HashSet<String> dataTypeDecl, Model model) {
        if (type instanceof DoubleDotRef) {
            DoubleDotRef ddrefType = (DoubleDotRef) type;
            if (ddrefType.getElm()
                    instanceof
                    DataImplementation) { // if it is a AADL data implementation definition
            ...
            } else if (ddrefType.getElm()
                    instanceof
                    org.osate.aadl2.DataType) { // if it is a AADL data implementation definition
                org.osate.aadl2.DataType aadlDataType =
                        (org.osate.aadl2.DataType) ddrefType.getElm();
                resolveAADLDataType(aadlDataType, dataTypeDecl, model);
            } else {
                System.out.println(
                        "Unresolved data type "
                                + ddrefType.getElm().getName()
                                + " in doubledotref. Not AADL DataImplementation or DataType type.");
            }
        } else {
    ...
    private String getDataTypeName(Type type) {
        ...
        if (type instanceof PrimType) {
            ...
            verdict.vdm.vdm_data.PlainType plaintype =
                    verdict.vdm.vdm_data.PlainType.fromValue(primType.getName());
    ...
    private void resolveAADLDataImplementationType(
            DataImplementation dataImplementationImpl, HashSet<String> dataTypeDecl, Model model) {
        verdict.vdm.vdm_data.DataType dtype = new verdict.vdm.vdm_data.DataType();
            ...
            for (DataSubcomponent dataSubComp : subcomponents) {
                ...
                if (dataSubCompType instanceof org.osate.aadl2.DataType) {
                    ...
                    verdict.vdm.vdm_data.DataType recFieldDtype =
                            getVdmTypeFromAADLType(aadlDataType);
                    ...
                    // the line below is just to ensure that the type's declaration is added to VDM
                    resolveAADLDataImplementationType(
                            dataSubCompDataImplementation, dataTypeDecl, model);
                    verdict.vdm.vdm_data.DataType recFieldDtype =
    ...
    public verdict.vdm.vdm_data.DataType getVdmTypeFromAADLType(
            org.osate.aadl2.DataType aadlDataType) {
        verdict.vdm.vdm_data.DataType dtype = new verdict.vdm.vdm_data.DataType();
    ...
    private void resolveAADLDataType(
            org.osate.aadl2.DataType aadlDataType, HashSet<String> dataTypeDecl, Model model) {
        verdict.vdm.vdm_data.DataType dtype = new verdict.vdm.vdm_data.DataType();
        if (aadlDataType.getName().contentEquals("Float")
                || aadlDataType.getName().contentEquals("Float_32")
                || aadlDataType.getName().contentEquals("Float_64")) {
        ...
        if (!dataTypeDecl.contains(aadlDataTypeName)
                && (!aadlDataTypeName.equalsIgnoreCase("Float"))
                && (!aadlDataTypeName.equalsIgnoreCase("Integer"))
                && (!aadlDataTypeName.equalsIgnoreCase("Boolean"))) {
     ...
     *     enum definition and gets information from the properties to define the enum in the VDM *
     */
    public boolean updateVDMDatatypeUsingProperties(
            verdict.vdm.vdm_data.DataType dtype, EList<PropertyAssociation> properties) {
        if (properties.size() == 2) {
            ...
            if (firstPropertyValues.size() == 1) {
                ...
                if (ownedval instanceof NamedValueImpl) {
                    ...
                    if (namedVal.getNamedValue() instanceof EnumerationLiteralImpl) {
                        EnumerationLiteral namedValEnumLit =
                                (EnumerationLiteral) namedVal.getNamedValue();
                        if (namedValEnumLit.getName().equalsIgnoreCase("Enum")) {
                            ...
                            EList<ModalPropertyValue> secondPropertyValues =
                                    secondProperty.getOwnedValues();
                            if (firstPropertyValues.size() == 1) {
                                PropertyExpression secPropValue =
                                        secondPropertyValues.get(0).getOwnedValue();
                                // enum should have multiple values so check if a list of values are
                                // defined
                                if (secPropValue instanceof ListValueImpl) {
                                    ListValueImpl listValueImpl = (ListValueImpl) secPropValue;
                                    EList<PropertyExpression> listOfValues =
                                            listValueImpl.getOwnedListElements();
                                    for (PropertyExpression enumvalue : listOfValues) {
                                        if (enumvalue instanceof StringLiteralImpl) {
                                            StringLiteralImpl stringEnumVal =
                                                    (StringLiteralImpl) enumvalue;
                                            vdmEnumType
                                                    .getEnumValue()
                                                    .add(stringEnumVal.getValue());
                                        } else {
                                            System.out.println(
                                                    "Unexpected non-string value for data type of type enum.");
                                        }
                                    }
                                    ...
                                    System.out.println(
                                            "The second property of the data definition is not of ListValueImp type");
                                }
                            } else {
                                System.out.println(
                                        "Unresolved data property. The first property of the data definition has no values or multiple values.");
                            }
                        } else {
                            System.out.println(
                                    "Unresolved data property value. Property's owned value's named value does not contain Enum");
                        }
                    } else {
                        System.out.println(
                                "Unresolved data property value. Property's owned value's named value is not EnumerationLiteralImpl type.");
                    }
                } else {
                    System.out.println(
                            "Unresolved data property value. Property's owned value is not NamedValueImpl type.");
                }
            } else {
            ...
            System.out.println(
                    "Unresolved data property. Data definition has 0 or more than 2 properties associated with it");
        }
        return false;
    }

    private ContractItem translateGuaranteeStatement(
            GuaranteeStatement guaranteeStmt,
            HashSet<String> dataTypeDecl,
            HashSet<String> nodeDecl,
            Model model) {
    ...
    private void addNodeDefToTypeDeclarations(
            NodeDef nodeDef, HashSet<String> dataTypeDecl, HashSet<String> nodeDecl, Model model) {
        ...
        for (Arg arg : nodeInpArgs) {
            ...
            if (!(argType instanceof PrimType)) {
                translateAgreeDataTypeToVdmDataType(argType, dataTypeDecl, model);
            } // this call is mainly to define the type if not already defined
        ...
        for (NodeStmt agreeNodeStmt : agreeNodeStmts) {
            if (agreeNodeStmt instanceof NodeEqImpl) {
                ...
                NodeEquationLHS vdmNodeEquationLHS =
                        new NodeEquationLHS(); // this type is just a list of strings
                ...
                // get the RHS i.e.expr of the agree NodeEq and set it as the vdm node equation's
                // RHS
                vdmNodeEquation.setRhs(
                        getVdmExpressionFromAgreeExpression(
                                agreeNodeEq.getExpr(), dataTypeDecl, nodeDecl, model));
    ...
    private Expression getVdmExpressionFromAgreeExpression(
        ...
        if (agreeExpr instanceof IfThenElseExpr) {
            ...
            Expression condExpr =
                    getVdmExpressionFromAgreeExpression(
                            ifexpr.getA(), dataTypeDecl, nodeDecl, model);
            ifThenElseVal.setCondition(condExpr);
            Expression thenBranchExpr =
                    getVdmExpressionFromAgreeExpression(
                            ifexpr.getB(), dataTypeDecl, nodeDecl, model);
            ifThenElseVal.setThenBranch(thenBranchExpr);
            Expression elseBranchExpr =
                    getVdmExpressionFromAgreeExpression(
                            ifexpr.getC(), dataTypeDecl, nodeDecl, model);
            ...
            if (ddref.getElm() instanceof NodeDef) {
                ...
                for (Expr callExprArg : callExprArgs) {
                    Expression argExpr =
                            getVdmExpressionFromAgreeExpression(
                                    callExprArg, dataTypeDecl, nodeDecl, model);
            ...
            if (nmExpr.getElm() instanceof Arg) {
                ...
                if (!dataTypeDecl.contains(nmElmConstStatementName)) {
                    ...
                    vdmConstDeclaration.setDefinition(
                            getVdmExpressionFromAgreeExpression(
                                    nmElmConstStatement.getExpr(), dataTypeDecl, nodeDecl, model));
                ...
                System.out.println(
                        "Unresolved/unmapped NamedExprElm: " + nmExpr.getElm().getName());
            }
            ...
            if (selExpr.getField() == null) {
                ...
                if (selExpr.getTarget() != null) { // target can be NamedElmExpr or a SelectionExpr
                    vdmRecordProj.setRecordReference(
                            getVdmExpressionFromAgreeExpression(
                                    selExpr.getTarget(), dataTypeDecl, nodeDecl, model));
                }
                ...
                if (field instanceof DataPortImpl) {
                    ...
                    if (argType instanceof PrimType) {
                        ...
                        System.out.println(
                                "Unresolved Arg Type so not setting record-type in record-projection.");
                    }
                } else {
            ...
            Expression leftOperand =
                    getVdmExpressionFromAgreeExpression(
                            binExpr.getLeft(), dataTypeDecl, nodeDecl, model);
            binoper.setLhsOperand(leftOperand);
            // set right operand
            Expression rightOperand =
                    getVdmExpressionFromAgreeExpression(
                            binExpr.getRight(), dataTypeDecl, nodeDecl, model);
            ...
            Expression singleOperand =
                    getVdmExpressionFromAgreeExpression(
                            unExpr.getExpr(), dataTypeDecl, nodeDecl, model);
            ...
            // check if elm is DataImplementationImpl or DataTypeImpl -- if yes add definition to
            // type declarations if not already present
            ...
            Expression expr =
                    getVdmExpressionFromAgreeExpression(
                            preExpr.getExpr(), dataTypeDecl, nodeDecl, model);
            vdmExpr.setPre(expr);
            ...
            if (recLitExpr.getRecordType() instanceof DoubleDotRef) {
                ...
                if (recType.getElm().getName() != null) {
                    // check if elm is DataImplementationImpl -- if yes add definition to type
                    ...
                    if (recType.getElm() instanceof DataImplementation) {
                        org.osate.aadl2.DataImplementation aadlDImpl =
                                (org.osate.aadl2.DataImplementation) recType.getElm();
                    ...
                    // get args and arg-expr and set them as field identifier and value in the vdm
                    // model
                    ...
                    for (int ind = 0; ind < recLitArgs.size(); ind++) {
                        ...
                        fieldDef.setFieldValue(
                                getVdmExpressionFromAgreeExpression(
                                        recLitArgsExpr.get(ind), dataTypeDecl, nodeDecl, model));
            ...
            if (eventExpr.getPort() != null) {
                vdmExpr.setEvent(
                        getVdmExpressionFromAgreeExpression(
                                eventExpr.getPort(), dataTypeDecl, nodeDecl, model));
            } else {
            ...
            vdmExpr.setToReal(
                    getVdmExpressionFromAgreeExpression(
                            realCastExpr.getExpr(), dataTypeDecl, nodeDecl, model));
        } else {
    ...
    private void defineDataTypeDataImplementationTypeInVDM(
            NamedElement nmElement, HashSet<String> dataTypeDecl, Model model) {
        if (nmElement instanceof DataTypeImpl) {
            ...
            org.osate.aadl2.DataImplementation aadlDImpl =
                    (org.osate.aadl2.DataImplementation) nmElement;
            resolveAADLDataImplementationType(aadlDImpl, dataTypeDecl, model);
        } else {
    ...
    private void defineDataTypeDataImplementationTypeInVDM(
        ...
        if (type instanceof PrimType) {
            ...
            defineDataTypeDataImplementationTypeInVDM(
                    typeDoubleDotRef.getElm(), dataTypeDecl, model);
        } else {
