...
public class GSNHandler extends AbstractHandler {
                    ...
                    new Thread() {
                        ...
                        public void run() {
                            try {
                                ...
                                /**
                                 * Create the xml model for the GSN creator in the GSN output
                                 * directory as modelXML.xml
                                 */
                                ...
                                if (outputsGenerated) {
                                    ...
                                    /**
                                     * The GSN creator backend needs: 1. The rootId 2. The Gsn
                                     * output directory 3. The Soteria++ Output directory 4. The
                                     * path of the project directory which contains the aadl files
                                    ...
                                    /**
                                     * save the xml model for the GSN creator in the GSN output
                                     * directory as modelXML.xml
                                     */
    ...
    /**
     * A function that decides which requirement Ids should be sent to the bundle Expected Behavior:
     * 1. Security disabled: a. If no input/typo/invalid -> Sends all mission reqs b. If valid
     * inputs -> Sends all input reqs 2. Security enabled: a. If all Ids specified are valid ->
     * sends all Ids (i) For every cyber req that is specified -> sends the cyber requirement (ii)
     * For every mission req that is specified -> sends the requirement and all supporting cyber
     * reqs b. If no input/typo/invalid -> sends all mission reqs and supporting cyber reqs
     *
     * @param model
     * @return
     */
    public String decideCorrectInput(Model model) {
        ...
        if (emptyFlag) {
            System.out.println(
                    "Warning: No user specified requirement. Generating for all mission requirements.");
            ...
            if (securityFlag) {
                ...
                for (String missionId : missionIds) {
                    supportingCyberForAllMissions.addAll(
                            getAllSupportingCyberReqs(model, missionId));
                    correctInput = correctInput + missionId + ";";
                }

                // removing duplicates from cupporting cybers
                List<String> duplicateFreeSupportingCyberForAllMissions =
                        new ArrayList<>(new HashSet<>(supportingCyberForAllMissions));
                ...
                if (securityFlag) {
                    if (missionFlag) { // otherwise add only provided mission Ids
                        ...
                        for (String missionId : userProvidedMissions) {
                            supportingCyberForUserProvidedMissions.addAll(
                                    getAllSupportingCyberReqs(model, missionId));
                        }

                        // removing duplicates from cupporting cybers
                        List<String> duplicateFreeSupportingCyberForUserProvidedMissions =
                                new ArrayList<>(
                                        new HashSet<>(supportingCyberForUserProvidedMissions));
                        ...
                        if (!cyberFlag) {
                            System.out.println(
                                    "Warning: Only safety requirements specified. Will ignore \"Generate Security Cases\" selection.");
                        }
                    }
                }
            } else {
                System.out.println(
                        "Warning: Ill formed input. Generating for all mission requirements.");
                // if security is enabled, input will have all mission reqs and all dependent cyber
                // reqs
                if (securityFlag) {
                    List<String> supportingCyberForAllMissions = new ArrayList<>();
                    for (String missionId : missionIds) {
     ...
     * @throws IOException
     */
    public static boolean runBundleMBAS(
            String bundleJar,
            String dockerImage,
            String projectName,
